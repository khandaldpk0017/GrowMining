        ...
        <div class="grid grid-cols-12 page-gap">          
          <div class="col-span-6">
            <div class="card">
              ...
              <div class="card-body"> 
                <form action="{{ route('admin.updateUser',['id' => $cuser->id]) }}" method="POST" role="form" enctype="multipart/form-data">
                  @csrf
                  <div class="form-group">                  
                    <input class="form-control" type="text" placeholder="Name" id="name" name="name" value="{{ old('name', $cuser->name) }}" required="required">
                    @error('name')
                                    <p class="text-danger">{{ $message }}</p>
                                    @enderror
                  </div>
                  <div class="form-group">                  
                    <input class="form-control" type="email" placeholder="Email" id="email" name="email" value="{{ old('email', $cuser->email) }}" required="required">
                    @error('email')
                                    <p class="text-danger">{{ $message }}</p>
                                    @enderror 
                  </div>
